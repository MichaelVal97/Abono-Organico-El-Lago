...
import {
  Theme,
  Language,
} from './../src/modules/auth/entities/user-preferences.entity';
    ...
    if (existingUser) {
      if (existingUser.preferences) {
        await userRepository.query(
          `DELETE FROM user_preferences WHERE id = '${existingUser.preferences.id}'`,
        );
      }
  ...
  afterAll(async () => {
    if (userRepository) {
      ...
      if (user) {
        ...
        if (user.preferences) {
          await userRepository.query(
            `DELETE FROM user_preferences WHERE id = '${user.preferences.id}'`,
          );
        }
    ...
    it('should update preferences', () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          ...
          expect(res.body.emailNotifications).toBe(
